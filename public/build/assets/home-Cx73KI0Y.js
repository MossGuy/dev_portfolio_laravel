const b={html5:{title:"HTML5",description:`Ik kan een nette HTML-structuur aanhouden door middel van de elementen nav, main, footer en section. Ik ben bekend met de basiselementen zoals linkjes, afbeeldingen, formulieren en tabellen. Ook ben ik bekend met favicon-icoontjes. 
    ...
    Als ik een webpagina ga maken, denk ik altijd zorgvuldig na over de layout volgens het mobile-first principe.`,images:["HTML5.png"]},cssFramework:{title:"CSS + Frameworks",description:`Ik gebruik nog vaak CSS om basiskleurenthema's op te slaan in de root-variabelen en zo een huisstijl te maken die makkelijk en snel gebruikt kan worden door alle pagina's in mijn project. 
      ...
      `,images:["Csharp.png"]},AspDotNet:{title:"ASP.NET",description:"Ik wijger om die bloat ooit nog aan te raken al geef je mij een duidelijke handleiding",images:["AspDotNet.png"]}};document.addEventListener("DOMContentLoaded",()=>{const r=document.querySelectorAll(".skill"),t=document.getElementById("ervaring_body"),o=t.innerHTML,l=window.assetPath||"/images";r.forEach(n=>{n.addEventListener("click",()=>{const d=n.dataset.skill,e=b[d];if(e){const m=e.title==="ASP.NET",i="p-6 mt-6 animate-fadeIn max-w-5xl mx-auto flex flex-col items-center justify-center gap-8",a=`
        ...
        `,s=(e.images??[]).map((g,k)=>`
              <img src="${l}/${g}" alt="${e.title} afbeelding ${k+1}"
            ...
            `;t.innerHTML=c,document.getElementById("backButton").addEventListener("click",()=>{t.innerHTML=o,document.dispatchEvent(new Event("DOMContentLoaded"))})}})})});
